...
### **Self-Hosted Remote Control System for Google Antigravity (`agy`) CLI & Developer Workstations (Windows, macOS, Linux) over Tailscale**

[![Python 3.12+](https://img.shields.io/badge/python-3.12+-blue.svg?logo=python&logoColor=white)](https://www.python.org/)
[![FastAPI](https://img.shields.io/badge/FastAPI-0.110+-009688.svg?logo=fastapi&logoColor=white)](https://fastapi.tiangolo.com/)
[![Cross-Platform](https://img.shields.io/badge/Host-Windows%20%7C%20macOS%20%7C%20Linux-0078D6.svg)](https://github.com/w1th0ut/GravityDesk)
...
*Control, prompt, and monitor autonomous Google Antigravity AI coding sessions on your developer workstation (Windows, macOS, or Linux) straight from your Android phone — wherever you are, zero cloud intermediaries required.*
...
- ⚡ **Zero-Lag Dual-Engine Virtual Terminal**: Cross-platform pseudo-terminal streaming (Windows ConPTY & Unix POSIX PTY) directly over WebSocket with a 3,000-chunk ring buffer for lossless session catch-up.
...
- 🪟 **Zero-Popup Headless Background Execution**: All background subprocesses (including `agy -p /usage` and audio transcoding) run silently without intrusive popup windows on the developer workstation.
...
- 🧭 **Multi-Drive Breadcrumb Workspace Navigator**: Explore and switch project folders effortlessly across filesystem roots (`C:\`, `D:\` on Windows, `/` and `/Volumes` on macOS/Linux) right from mobile.
...
- 🔋 **Power Management & Sleep Inhibit**: Prevents host laptops from entering standby or suspend mode during active remote sessions.
...
*Modern dark-mode dashboard with dynamic QR pairing, hardware vitals, active session management, and instant access revocation.*
...
| *Real-time host vitals & Gemini/Claude quota tracking* | *Zero-lag virtual terminal, breadcrumb nav & voice STT* |
...
| Layer | Technologies |
|---|---|
| **Host Workstation** | Windows 10 / 11, macOS, Linux, Python 3.12+, Tkinter, Pillow, `ffmpeg` |
| **Backend Daemon** | FastAPI, Uvicorn, `pywinpty` (Windows) / `pty` (Unix), `psutil`, `qrcode`, `SpeechRecognition`, SQLite |
...
- **Developer Workstation**: Windows 10/11, macOS, or Linux.
- **Python**: Version `3.12+` installed and added to `PATH`.
- **Google Antigravity**: `agy` CLI installed globally and authenticated.
- **Tailscale**: Installed and signed in on both your host workstation and your Android phone on the same tailnet.
...
If you want to use the hands-free voice-to-text dictation feature from your Android phone, install FFmpeg on your host workstation:
...
2. **Connect Tailscale**: Ensure both your host workstation and your Android phone are signed into the same Tailscale network.
3. **Pair Instantly**:
...
- **Binary Allowlist**: Spawning arbitrary executables is prohibited. Only pre-vetted shells (`bash`, `zsh`, `sh`, `cmd.exe`, `powershell.exe`) and Google Antigravity (`agy`, `agy.exe`) are permitted.
- **Path Traversal Protection**: Folder browsing requests are sanitized via `os.path.realpath`, validating paths against filesystem roots (`C:\`, `D:\` on Windows, `/` and `/Volumes` on macOS/Linux) to block traversal exploits.
- **Headless Process Sandboxing**: Background executions run with `CREATE_NO_WINDOW` (on Windows) and concurrency mutex locks to prevent desktop UI disruption and runaway process cascades.
...
| **Workspace Selector** | Interactive multi-drive breadcrumb navigation modal (`C:\`, `D:\` on Windows, `/` on macOS/Linux) to effortlessly switch project folders without touching your workstation. |
...
GravityDesk includes an automated integration test suite verifying authentication entropy, PTY process lifecycle, path traversal boundaries, and endpoint security:
...
│   ├── system.py              # Telemetry vitals & sleep inhibitor
...
│   └── test_server.py         # Security, PTY, and endpoint harness
...
- [x] Dual-engine terminal runner (Windows ConPTY & Unix POSIX PTY) with async WebSocket streaming
...
- [x] Multi-OS host support (Windows, macOS & Linux)
